...
print("=== CARICAMENTO DATASET RICETTE ===")
df = pd.read_csv('data/ricette_reali.csv', on_bad_lines='skip')
print(f"Dimensioni dataset: {df.shape}")

# Rimuovi la colonna ingredienti
if 'ingredienti' in df.columns:
    df = df.drop('ingredienti', axis=1)
    print("Colonna 'ingredienti' rimossa")

# Analisi preliminare del target
print("\n=== ANALISI TARGET (calorie_per_porzione) ===")
target_col = 'calorie_per_porzione'
...
numeric_features = ['numero_ingredienti', 'numero_porzioni', 'tempo_preparazione_min', 
                   'tempo_cottura_min', 'costo_stimato_euro', 'rating_medio', 
                   'numero_preparazioni', 'tempo_totale_min']

categorical_features = ['tipo_cucina', 'difficolta', 'tipo_piatto', 'metodo_cottura', 
...
with open('scaler_ricette.pkl', 'wb') as f:
    pickle.dump(scaler, f)
with open('label_encoders_ricette.pkl', 'wb') as f:
...
with open('best_regression_models_ricette.pkl', 'wb') as f:
    pickle.dump(best_models, f)
with open('regression_configs_ricette.pkl', 'wb') as f:
    pickle.dump(best_configs, f)
with open('regression_results_ricette.pkl', 'wb') as f:
...
results_df.to_csv('regression_comparison_results_ricette.csv')
...
for i, metric in enumerate(metrics):
    ...
    ax.set_title(f'Confronto {metric} - Ricette')
...
plt.tight_layout()
plt.savefig('regression_metrics_comparison_ricette.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.xlabel('Valori Reali (calorie)')
plt.ylabel('Predizioni (calorie)')
...
plt.xlabel('Predizioni (calorie)')
plt.ylabel('Residui')
plt.title(f'Residui - {best_model_name}')
plt.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(f'best_model_analysis_ricette_{best_model_name.lower()}.png', dpi=300, bbox_inches='tight')
plt.close()
...
def plot_learning_curve_regression(estimator, title, X, y, cv=5, train_sizes=np.linspace(.1, 1.0, 10)):
    ...
    plt.ylabel("MSE")
    plt.title(f'Learning Curve Ricette - {title}')
    plt.legend(loc="best")
    ...
    plt.tight_layout()
    # Nome specifico per Random Forest, standard per altri
    if 'random' in title.lower():
        filename = f'learning_curve_ricette_{title.lower().replace(" ", "_")}_regressor.png'
    else:
        filename = f'learning_curve_ricette_{title.lower().replace(" ", "_")}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Learning curve salvata: {filename}")
...
if 'RandomForest' in best_models:
    ...
    plt.title('Feature Importance Ricette - Random Forest')
    ...
    plt.tight_layout()
    plt.savefig('feature_importance_ricette_rf.png', dpi=300, bbox_inches='tight')
    plt.close()
...
print("- best_regression_models_ricette.pkl: Modelli addestrati")
print("- regression_configs_ricette.pkl: Configurazioni ottimali")
print("- regression_results_ricette.pkl: Risultati dettagliati")
print("- regression_comparison_results_ricette.csv: Tabella comparativa")
print("- scaler_ricette.pkl: Standardizzatore per preprocessing")
print("- label_encoders_ricette.pkl: Encoder per variabili categoriche")
print("- Grafici salvati come PNG con suffisso _ricette")
